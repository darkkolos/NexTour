 ...
 * guias pesquisados por um utilizador.
 */
public class PaginaPesquisar extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener, BottomNavigationView.OnNavigationItemSelectedListener {

    private static final String TAG = "Pagina pesquisar";
    ...
    private EditText nomeGuia;
    private EditText localidadeGuia;
    private LinearLayout contentorInicial;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pagina_pesquisar);

        BottomNavigationView navigationView = (BottomNavigationView) findViewById(R.id.navigationView);
        navigationView.setOnNavigationItemSelectedListener(this);
        navigationView.setSelectedItemId(R.id.paginaPesquisar);
        ...
        contentorInicial = findViewById(R.id.contentor_incial);

        localidadeGuia = findViewById(R.id.input_pequisa_localidade);
        nomeGuia = findViewById(R.id.input_pequisa_titulo);
    ...
    private void mostrarContentor() {
        contentorInicial.setVisibility(View.VISIBLE);
    }

    private void esconderContentor() {
        contentorInicial.setVisibility(View.GONE);
    }
    ...
    private void carregarGuias() {
        ...
        StringRequest strRequest = new StringRequest(Request.Method.GET, "http://"
                ...
                + getResources().getString(R.string.rest_key) + "/guias/pesquisa?titulo=" + nomeGuia.getText().toString().trim()
                + "&localidade=" + localidadeGuia.getText().toString().trim(), new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                Gson gson = new Gson();
                ArrayList<GuiaTuristico> listaGuiasTuristicos = gson.fromJson(response, new TypeToken<ArrayList<GuiaTuristico>>() {
                }.getType());
                if (listaGuiasTuristicos.size() > 3) {
                    //esconderContentor();
                }

                if (listaGuiasTuristicos == null) {
                    Toast.makeText(getApplicationContext(), "Não foram encontrados guias com esses termos de pesquisa", Toast.LENGTH_LONG).show();
                } else {
                    ListaGuiasListAdapter listaGuiasListAdapter = new ListaGuiasListAdapter(getApplicationContext(), R.layout.adapter_view_ver_guias, listaGuiasTuristicos, getSupportFragmentManager(), false);
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                params.put("titulo", nomeGuia.getText().toString().trim());
                params.put("localidade", localidadeGuia.getText().toString().trim());
                return params;
            }
        };
